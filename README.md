...
- `/start` — начать
- Голосовое, кружочек, аудио или видео — бот сразу пришлёт расшифровку `.txt`
  и предложит, что сделать с текстом

**Большие файлы (до 2 ГБ).** Обычный Bot API отдаёт ботам файлы только до 20 МБ.
Если указать в `.env` `TELEGRAM_API_ID` и `TELEGRAM_API_HASH` (выдаются бесплатно
на https://my.telegram.org → API development tools), бот скачивает большие файлы
через MTProto тем же токеном. Без них на большой файл бот ответит ссылкой
`WEB_TRANSCRIBE_URL`, если она задана.

**Через VPN.** Если Telegram или Groq с сервера напрямую недоступны, укажите
локальный прокси VPN: `TELEGRAM_PROXY=socks5://127.0.0.1:1080` и
`GROQ_PROXY=socks5://127.0.0.1:1080`.
...
**Результат:** расшифровка `.txt` сразу, потом PDF по каждому выбранному режиму
